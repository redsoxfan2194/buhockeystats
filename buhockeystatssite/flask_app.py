...
def dailyTrivia():
    ...
    launchSeed=int('2023'+'244')
    seedVal=int(datetime.datetime.now(easternTZ).strftime('%Y')+datetime.datetime.now(easternTZ).strftime('%j'))
    random.seed(seedVal)
...
def getJacksBoxesGameNum():
  ...
  df['date'] = pd.to_datetime(df['date'], format='%Y-%m-%d') + pd.Timedelta(hours=6)
